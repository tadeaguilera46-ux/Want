  ...
  const handleCreate = async () => {
    ...
    try {
      await createMenuItem(restaurantId, {
        ...
        image: image.trim(),
        active: true,
        station: (station.trim() || null) as string | undefined,
        drinkType: (type === "drinks" ? drinkType : null) as typeof drinkType | undefined,
        ingredients,
        variants: [],
  ...
  const saveEditing = async (item: MenuItem) => {
    ...
    try {
      ...
      await updateMenuItem(restaurantId, item.id, {
        ...
        station: (draft.station?.trim() || null) as string | undefined,
        drinkType: (draft.type === "drinks" ? (draft.drinkType || "cocktail") : null) as typeof draft.drinkType | undefined,
        ingredients: draft.ingredients,
        availableFrom: (draft.availableFrom?.trim() || null) as string | undefined,
        availableTo: (draft.availableTo?.trim() || null) as string | undefined,
        availableDays: (draft.availableDays?.length ? draft.availableDays : null) as number[] | undefined,
        allergens: (draft.allergens?.length ? draft.allergens : null) as string[] | undefined,
        modifierGroups: (draft.modifierGroups?.length ? draft.modifierGroups : null) as typeof draft.modifierGroups | undefined,
